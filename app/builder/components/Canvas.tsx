...
const NodeRenderer = ({ node }: { node: ComponentNode }) => {
  ...
  const hasOwnChildrenProp = Object.prototype.hasOwnProperty.call(
    node.props,
    "children",
  );
  ...
  const resolvedClassName =
    selectedCustomStyle?.className || node.props.className || "";
  ...
  const onResizeMove = (event: MouseEvent) => {
    ...
    if (stateRef.handle === "bottom" || stateRef.handle === "corner") {
      nextHeight = Math.max(
        minSize,
        Math.round(stateRef.initialHeight + deltaY),
      );
    }
  ...
  if (hasChildNodes) {
    componentProps.children = node.children.map((child) => (
...
export const Canvas = () => {
  ...
  const currentPopup = state.popups.find(
    (popup) => popup.id === state.currentPopupId,
  );
  const activeNodes =
  ...
  if (state.editingTarget === "page" && !currentPage)
    return <div>No page selected</div>;
  if (state.editingTarget === "popup" && !currentPopup)
    return <div>No popup selected</div>;
